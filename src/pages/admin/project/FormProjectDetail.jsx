...
const FormProjectDetail = () => {
  ...
  const onFinish = (values) => {
    ...
    if (values?.detail_image) {
      formData.append("name", oldData?.name);
      formData.append("file", values?.detail_image[0]?.originFileObj);
      axios
        .post(
          `${process.env.REACT_APP_API}/project/updateProjectDetail/` + oldData?.id,
          formData,
          {
            headers: {
  ...
  const getProject = async (projectName) => {
    ...
    if (res?.detail_image) {
      console.log("res?.detail_image?.url", res?.detail_image?.url);
      setLogoFileList([
        {
          uid: "-1",
          name: "image.png",
          status: "done",
          url: res?.detail_image?.url,
        },
      ]);
      setPreviewImage(res?.detail_image?.url);
    }
    console.log("res", res);
    setOldData(res);
  };
...
export default FormProjectDetail;
